    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, w=None):
        ...
        conv = keras.layers.Conv2D(
            ...
            kernel_initializer=keras.initializers.Constant(
                w.conv.weight.permute(2, 3, 1, 0).detach().numpy()
            ),
            bias_initializer="zeros"
            if hasattr(w, "bn")
            else keras.initializers.Constant(w.conv.bias.detach().numpy()),
        )
...
class TFBottleneck(keras.layers.Layer):
    # Standard bottleneck
    def __init__(
        self, c1, c2, shortcut=True, g=1, e=0.5, act=True, w=None
...
class TFConv2d(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, k, s=1, g=1, bias=True, w=None):
        ...
        self.conv = keras.layers.Conv2D(
            ...
            bias_initializer=keras.initializers.Constant(w.bias.detach().numpy())
            if bias
            else None,
        )
...
class TFC3(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, act=True, w=None):
        ...
        self.m = keras.Sequential(
            [
                TFBottleneck(c_, c_, shortcut, g, e=1.0, act=act, w=w.m[j])
                for j in range(n)
            ]
        )
...
class TFC3x(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, act=True, w=None):
        ...
        self.m = keras.Sequential(
            [
                TFCrossConv(
                    c_, c_, k=3, s=1, g=g, e=1.0, shortcut=shortcut, act=act, w=w.m[j]
                )
                for j in range(n)
            ]
        )
...
class TFDetect(keras.layers.Layer):
    ...
    def call_onnx(self, inputs):
        ...
        for i in range(self.nl):
            ...
            x[i] = tf.reshape(x[i], [-1, ny * nx, self.na, self.no])
            z.append(
                tf.reshape(x[i], [-1, self.na * ny * nx, self.no])
            )  # Mehrdad: Onnx
